...
Agent 的职责是协作默认值，不是固定身份、独占所有权或能力边界。项目所有者可以根据任务范围、
风险、上下文和可用性，让任一 Agent 承担设计、实现、诊断或评审；真正约束行为的是当前任务角色
和授权范围，而不是 Agent 名称。
...
- Codex 主要侧重需求澄清、方案与架构设计、任务协调，以及部分代码实现。
- Cursor 主要侧重代码和规格评审，也可以在当前任务明确分配时参与设计、诊断或实现。
- Claude 主要侧重代码实现和评审，也可以按任务需要参与需求、架构与方案设计。
- 上述侧重不禁止交叉承担任务，也不使任何 Agent 自动获得写入、批准、合并或独立评审资格。

### 实现任务

- 任一被分配为实现者的 Agent 都可以在已批准范围内修改代码、测试和权威文档。
- 实现者负责检查现有接口、测试、失败模式和工作树重叠，不把未批准提案写成既定事实，不擅自
  扩大范围，也不以代码生成代替验收证据。
- 是否由一个或多个 Agent 实现，依据任务风险和路径所有权决定，不按 Agent 名称永久分配模块。

### 评审任务

- 任一被分配为正式评审方的 Agent 在该次评审中默认只读，依据固定 revision 或显式工作树快照
  判断；这项只读约束属于评审任务，不属于 Cursor、Claude 或其他 Agent 的永久身份。
- 评审方形成自己的结论前，不读取另一评审方尚未定稿的原始报告；结论趋同不降低取证强度，
  结论分歧也不通过修改对方状态来对齐。
- Agent 可以检查自己实现的代码并报告问题，但必须标记为自审，不能把它计作独立评审；要求
  独立评审时，应由未参与该范围实现的 Agent 对固定 revision 取证。
- 评审方不直接批准需求、架构或决策，也不在同一评审任务内修改被评审实现；若用户随后要求其
  修复，应结束评审角色并作为新的实现任务处理。
