...
本文的概念分成三条相互连接但所有权不同的链。先区分这三条链，再阅读后续状态机和检索流程：
...
知识内容链
Source Document（JDAgent 外部）
  → Knowledge Source（稳定逻辑身份）
  → Source Version（不可变内容版本）
  → Raw Source Artifact（原始字节）
  → Parsed Source Snapshot（指定 Parser/Locator 的派生结构）
  → Parent Segment / Child Chunk
  → Index Generation + Content Revision（可重建检索投影）
...
Knowledge Base 是管理来源生命周期的业务容器，不等于 Milvus Collection。Milvus 只保存知识内容链
末端的可重建投影；Session 只记录某一 Turn 实际使用了什么，不拥有知识库配置。Turn Retrieval
