...
**来源版本（Source Version）**：
一个 Knowledge Source 在一次成功摄取中形成的不可变内容版本，拥有内容哈希、Raw Source
Artifact 引用和生命周期。Parser 或 Locator 规则变化不制造新的 Source Version，而是从同一版本
产生新的 Parsed Source Snapshot。
...
特定 Parser Profile 与 Locator Schema 从一个 Source Version 派生的不可变规范结构。一个 Source
Version 可以拥有多个 Snapshot；Parent Segment、Child Chunk 和 Index Generation 必须指明使用哪一个。
