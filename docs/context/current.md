...
`Implemented（工程复核未通过，学习验证已完成）`。P1 核心代码已合入 `0.3.0`；M0 需求/合同/Eval
...
`5254905` 之后的上一轮修复已提交为 `ac17c40`；2026-09-05 审查修复随后已提交为
`657ecf53df3254041d17a66e1b130ea2d71c8075`（2026-09-13 核对 Git）。
...
处置；人工学习出口已于 2026-09-16 由项目所有者裁定关闭。
...
独立实现评审 High 已处置。M0 详细合同仍保持 `Proposed`；项目所有者于 2026-09-16 裁定
v0.3 学习验证完成，详见[学习完成记录](../learning/v0.3-rag-learning-checklist.md)。

项目所有者于 2026-09-16 决定下一模块为 v0.4 Tool Registry & Function Calling，扩展可调用工具
并完善相关模块设计；长期记忆与 Context / Compaction 依次顺延为 v0.5、v0.6，MCP 仍为后续
候选。截至 2026-09-23，v0.4 设计访谈收敛于 Q1–Q23，已形成
[设计提案](../architecture/v0.4-tool-execution.md)、术语表与专属学习清单，均为 `Proposed`，
整体设计尚未批准，未开始产品实现。Q23 已裁定按实际依赖判断 v0.3 遗留项的阻塞范围；
无关 RAG 缺口独立跟踪，不再统一阻塞 v0.4，v0.3 工程验收状态保持原记录。
...
- 项目所有者于 2026-09-16 调整[v0.2 之后的模块学习路线](../plans/post-v0.2-learning-roadmap.md)：
  v0.3 RAG 之后优先推进 v0.4 Tool Registry & Function Calling，原 Memory 与 Context 模块依次
  顺延为 v0.5 长期记忆生命周期、v0.6 Context Orchestration 与 Compaction；MCP 保留为后续候选。
  v0.4 基于已有三工具与调用闭环扩展实用工具、完善模块边界，为这些后续模块提供基础。
- v0.4 已形成待审阅设计提案。项目所有者于 2026-09-23 在
  [Q23](../plans/v0.4-tool-execution-design-discussion.md) 选择按依赖划分跨版本工程门禁：影响
  工具执行、权限、Session 和基础回归的遗留问题须在相关实现前处理；无关 RAG 缺口独立跟踪。
  此裁定不豁免新回归、不批准整体设计、不授权实现，也不表示 v0.3 工程验收通过。
- v0.3 高层设计与实施计划已获批，P1 工程实现已合入 `0.3.0`，学习出口已于 2026-09-16
  由项目所有者裁定关闭。
- v0.3 的 M0 详细需求、合同、Eval 规格仍为 `Proposed`；学习完成裁定不改变这些设计包的审批状态。
...
- 对 `5254905` 的自审修复已在 `ac17c40`；2026-09-05 七类问题的修复已在 `657ecf5`。完整验证属于自测；
  另有只读 Agent 对本轮改动做回归复核，发现并闭环两个追加问题，不代表全部 v0.3 独立验收通过。
- 未关闭：M0 设计包仍为 `Proposed`；Milvus Standalone Live；修复 SHA 的独立复核（可选）；
  历史延期项 F09/F11/F13/F16/F17 与 `credential_ref` 解析仍未做；本轮又确认恢复、
...
- 项目所有者于 2026-09-16 明确裁定：“当前视作 v0.3 已经完成了学习验证。”学习出口据此关闭。
- [学习记录](../learning/v0.3-rag-learning-checklist.md)保留原清单作为可选复习材料；未勾选项
  不再构成学习门禁，也不表示所有原定练习或延迟复测均已执行。
- 本裁定不改变上述工程状态、M0 设计包审批或后续模块的通用工作流。

## 下一步

审阅 v0.4 [设计提案](../architecture/v0.4-tool-execution.md)与
[专属学习清单](../learning/v0.4-tools-learning-checklist.md)：23 题范围取舍已记录，具体字段、
...
手动体验与验证可按[操作报告](../development/v0.3-rag-manual-verification.md)执行；该报告为 `Proposed`，
不表示人工验收完成。2026-09-13 已演练隔离的本地 CLI/检索流程，未重跑真实服务或完整历史验收套件。
...
4. 在人工环境矩阵中继续观察中文 IME、legacy console 与真实窗口强关；强关不承诺执行 finally，
