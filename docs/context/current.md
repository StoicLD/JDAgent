...
候选。当前进入 v0.4 的需求与设计准备，具体设计包尚未形成；路线调整不改变 v0.3 工程验收状态。
...
- v0.4 当前推进初始模型、需求与设计准备；具体工具清单、Interface、Eval 和实施计划在模块设计包
  中确定。进入实质性实现前仍须核对工程门禁；v0.3 未关闭项对 v0.4 的阻塞范围若需调整，由项目
  所有者明确裁定，不把此次路线调整视为 v0.3 工程验收通过。
...
优先推进 v0.4 Tool Registry & Function Calling 的需求与设计准备，按
[模块路线](../plans/post-v0.2-learning-roadmap.md)和[交付工作流](../development/module-delivery-workflow.md)：
先由学习者形成初始模型，再比较新增工具场景、注册与执行边界、结果合同和失败模式，形成包含
验收标准、最小 Eval、实施计划与专属学习清单的设计包。设计中核对依赖的 v0.3 工程缺口，明确
实质性实现前的处置与启动条件。
